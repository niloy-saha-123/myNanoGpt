"""
Generalization Test: Copy Task (5 letters)

Same format as base: INPUT<sep> -> OUTPUT.
5 letters (A..E), 5 positions. Input space: 5^5 = 3,125.

Shows progression: 3-letter -> 4-letter (100%) -> 5-letter -> 8-letter.

Usage:
  python generalize_copy_5letter.py           # default 500 train
  python generalize_copy_5letter.py 1000      # 1k train
"""
...
def make_dataset(examples):
    ...
    for tup in examples:
        ...
        for i in range(SEQ_LEN, len(tgt)):  # output positions in tgt
            masked_tgt[i] = tgt[i]
        inputs.append(inp)
...
def build_hand_copy():
    ...
    with torch.no_grad():
        ...
        for t in range(SEQ_LEN + 1, block_size):
            W_K[vocab_size + t, vocab_size + (t - SEQ_LEN - 1)] = 100.0
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("  GENERALIZATION TEST: Copy Task (5 letters)")
    print("=" * 60)
